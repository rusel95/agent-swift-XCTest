...
### Added
- **Launch UUID from the test bundle's `Info.plist` (`ReportPortalLaunchUUID`)**, with the
  same precedence as the existing `RP_LAUNCH_UUID` environment variable (env first). Real-device
  farms never deliver environment variables to the XCUITest process — on SauceLabs `env` is
  documented as *Virtual Devices Only* — and the `.xctestrun` is regenerated by the farm, so the
  compiled-in plist is the only channel that reaches every shard. With it set, all shards (and
  any farm-level retry re-running the same bundle) converge on one launch: the first
  `POST /launch` creates it, the rest join on HTTP 409. The post-run merge step is no longer
  needed, by @rusel95.
  - The value is used as the launch id verbatim, so any run-unique string works and CI can
    pass an identifier it already has (e.g. `${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}`).
  - `ReportPortalSkipFinish` now defaults to **on** when the launch UUID came from the plist:
    that launch is shared, and ReportPortal force-finishes every still-running item as
    `INTERRUPTED` when a launch is finished, so the run must be closed once, from CI, after all
    shards are done. An explicit `ReportPortalSkipFinish` still wins.
